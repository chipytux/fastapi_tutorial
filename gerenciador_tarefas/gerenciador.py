...
app = FastAPI(docs_url='/', title="TODO List", description="Implementa um Gerenciador de Tarefas TODO List")
...
def remover(id: UUID) -> None:
    ...
    tarefa_a_ser_removida = list(
        [elem for elem in TAREFAS if elem['id'] == str(id)])
...
def finalizar(id: UUID, entrada: FinalizarEntrada):
    '''
...
@app.get('/tarefas/{id}', name='Detalhar Tarefa', tags=['Tarefas'])
def detalhar(id: UUID):
    ...
    try:
        tarefa_encontrada = next(
            filter(lambda tarefa: tarefa['id'] == idstr, TAREFAS))
    except:
        raise HTTPException(status_code=404)
    else:
